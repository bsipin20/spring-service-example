...
public class ServiceApplication {
	@Bean
	ApplicationListener <ApplicationReadyEvent> readyEventApplicattionListener(CustomerService service) {
		return e -> {
...
class BracketService {
	...
	private final RowMapper <Bracket> bracketRowMapper =
		(rs, i) -> new Bracket(rs.getInt("id"), rs.getString("name"));
	...
	Collection <Bracket> findAll() {
		return this.template.query("select * from brackets", (rs, i) -> new Bracket(rs.getInt("id"), rs.getString("name")));
	}
}
...
record Bracket (Integer id, String name) {}
